 ...
 *
 * <p>We use Mockito to replace real dependencies (UserRepository, PasswordEncoder, etc.) with test
 * doubles — this isolates the service logic from the database and other infrastructure
 * concerns. @ExtendWith(MockitoExtension.class) — activates Mockito annotations
 * (@Mock, @InjectMocks) @Mock — creates a mock (fake implementation that records calls and returns
 * configured values) @InjectMocks — creates the class under test and injects mocks into its
 * constructor
 */
...
class AuthServiceTest {
  ...
  /**
   * @BeforeEach runs before every test method. Initialises shared test data so each test starts in
   * a known state.
   */
  @BeforeEach
  void setUp() {
