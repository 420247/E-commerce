...
/**
 * Integration tests for AuthController. @WebMvcTest spins up only the web layer (controllers,
 * filters, security config) without starting a full ApplicationContext or connecting to a database.
 * This makes the tests fast while still verifying HTTP behaviour.
 *
 * <p>MockMvc lets us send HTTP requests and assert on the response without starting a real server.
 */
