...
/**
 * REST controller for authentication endpoints. Handles user registration and login — returns JWT
 * tokens on success. Base URL: /api/auth
 */
...
public class AuthController {
  ...
  /**
   * POST /api/auth/register Creates a new user account and returns JWT tokens. @Valid — runs
   * validation from RegisterRequest (@NotBlank, @Email, @Size) @RequestBody — parses JSON request
   * body into RegisterRequest object
   */
  ...
  /**
   * POST /api/auth/login Authenticates user by email/password and returns JWT tokens. Returns 401
   * Unauthorized automatically if credentials are wrong.
   */
  @PostMapping("/login")
