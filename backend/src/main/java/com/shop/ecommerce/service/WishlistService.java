...
/**
 * Service for wishlist business logic. All operations require authenticated user — userId extracted
 * from JWT in controller. @Transactional — ensures DB operations are atomic (all succeed or all
 * roll back)
 */
@Service
...
public class WishlistService {
  ...
  /**
   * Returns all wishlist items for a given user. Called when user opens their wishlist page.
   *
  ...
  /**
   * Adds a product to user's wishlist. Throws exception if product already in wishlist or
   * product/user not found.
   *
  ...
  /**
   * Removes a product from user's wishlist. Silent if product was not in wishlist.
   *
   * @param userId extracted from JWT token
